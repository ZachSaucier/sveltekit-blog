...
Only Chrome supports this more complicated rendering inside of console as of this time.
...
From there I added a CSS keyframe animation. It also worked! So I already knew that I could use it to animate the SVG inside of the console.
...
`foreignObject` works!! But only if `xmlns="http://www.w3.org/1999/xhtml"` is set on the outermost element. Also note that the HTML content has the same sort of restrictions that
